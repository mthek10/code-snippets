...
public class AnimationReader {
  ...
  private static boolean THETA_ENABLED = false;
  private static boolean LAYER_ENABLED = false;
  ...
  public static <Doc> Doc parseFile(Readable readable, AnimationBuilder<Doc> builder) {
    ...
    // reset theta in case reader is reused
    THETA_ENABLED = false;

    // reset layer in case reader is reused
    LAYER_ENABLED = false;
  ...
  private static <Doc> void readShape(Scanner s, AnimationBuilder<Doc> builder) {
    ...
    if (LAYER_ENABLED) {
      if (s.hasNextInt()) {
        ...
        throw new IllegalStateException("Shape: Layer is enabled, Expected a" +
                " layer but no more int available");
      }
    }
  ...
  private static <Doc> void readMotion(Scanner s, AnimationBuilder<Doc> builder) {
    String[] fieldNames = new String[]{"initial time",
        ...
        "initial red value", "initial green value", "initial blue value", "initial theta",
        "final time",
        "final x-coordinate", "final y-coordinate",
        "final width", "final height",
        "final red value", "final green value", "final blue value", "final theta",
    };
    int[] vals = new int[18];
    String name;
    if (s.hasNext()) {
    ...
    for (int i = 0; i < 18; i++) {
      // if theta is not defined then set theta1 and theta2 to 0 default
      if (!THETA_ENABLED && (i == 8 || i == 17)) {
        vals[i] = 0;
      } else {
